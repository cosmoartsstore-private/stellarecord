...
- `theme.ts` — テーマ定義と LocalStorage I/O
...
| テーマ | `localStorage` | `localStorage["stella-theme"]` |

`save_management_settings(startup_enabled, archive_limit_mb)` で容量と自動起動を一括保存する。容量は最低 1 MB に正規化される。
...
| State | Persistence |
| ----- | ----------- |
| Theme | LocalStorage `stella-theme` |
...
`src-tauri/src/analyze/parser.rs` で定義された 23 個の正規表現を `parse_and_import_reader` の行単位ループ内で順次マッチさせる。
...
過去の調査で問題なしと判定された項目は [`fix-task.md`](../fix-task.md) を参照。
