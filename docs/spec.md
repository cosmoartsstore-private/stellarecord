...
| 取り込みパーサーが非 UTF-8 行をスキップする                                                                              | 破損バイトを含む行のイベントは DB に登録されない                    | ログビューアは U+FFFD 置換で表示継続。DB 取り込みで破損行を保存対象にする場合はバイト列ベースのデコーダーへ拡張 |
...
過去の調査で問題なしと判定された項目と再確認済みの非問題は、リポジトリルートの `AGENT.md` を参照。
